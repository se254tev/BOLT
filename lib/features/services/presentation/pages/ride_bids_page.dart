...
class RideBidsPage extends ConsumerStatefulWidget {
  final String requestId;

  const RideBidsPage({required this.requestId, super.key});

  @override
  ConsumerState<RideBidsPage> createState() => _RideBidsPageState();
}

class _RideBidsPageState extends ConsumerState<RideBidsPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Worker selected! Ride in progress.')),
        );
        context.go('/services/rides/status/${widget.requestId}');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('No riders available yet', style: AppTextStyles.h2),
                        const SizedBox(height: 8),
                        const Text('Waiting for riders to submit bids...'),
                      ],
                    ),
                  );
                }
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text(bid.workerName ?? 'Rider', style: AppTextStyles.h2),
                                          if (bid.rating != null)
                                            Text(
                                ...
                                if (bid.vehicleType != null) ...[
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text(bid.vehicleType!, style: const TextStyle(fontSize: 12)),
                                    ],
                                  ),
                                ],
