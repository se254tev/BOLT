...
class RideStatusPage extends ConsumerStatefulWidget {
  ...
  const RideStatusPage({required this.requestId, super.key});
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ride completed! Leave a review')),
        );
        context.go('/services/rides/review/${widget.requestId}');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _cancelRide() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        context.go('/services/rides/create');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Column(
            ...
            children: [
              ...
              if (request.status != RequestStatus.cancelled) ...[
                const Text('Ride Progress', style: AppTextStyles.h2),
                const SizedBox(height: 16),
                _buildTimelineItem('Request Created', true),
                _buildTimelineItem('Riders Bidding', request.status != RequestStatus.open),
                _buildTimelineItem('Rider Selected', request.status.index >= RequestStatus.assigned.index),
                _buildTimelineItem('Ride Completed', request.status == RequestStatus.completed),
                const SizedBox(height: 24),
              ],
              // Request details
              AppCard(
              ...
              if (request.status != RequestStatus.completed && request.status != RequestStatus.cancelled)
                OutlinedButton(
                  ...
                  child: Text(_isCancelling ? 'Cancelling...' : 'Cancel Ride'),
                ),
            ],
          ),
        ),
